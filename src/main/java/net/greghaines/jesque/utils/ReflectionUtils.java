...
public final class ReflectionUtils {
  ...
  static {
    ...
    final List<Class<?>> commonClasses =
        Arrays.asList(
            Boolean[].class,
            Byte[].class,
            Character[].class,
  ...
  /**
   * Create an object of the given type using a constructor that matches the supplied arguments and
   * invoke the setters with the supplied variables.
   *
   * @param <T> the object type
   * @param clazz the type to create
   * @param args the arguments to the constructor
  ...
  public static <T> T createObject(
      final Class<T> clazz, final Object[] args, final Map<String, Object> vars)
      throws NoSuchConstructorException,
          AmbiguousConstructorException,
          ReflectiveOperationException {
   ...
   * the ClassLoader that loaded the ReflectionUtils class will be used as fallback.
   *
   ...
   *
   * <p>Always uses the default class loader: that is, preferably the thread context class loader,
   * or the ClassLoader that loaded the ReflectionUtils class as fallback.
   *
    ...
    if (instance != null && vars != null) {
      ...
      for (final Entry<String, Object> entry : vars.entrySet()) {
        final String methodName =
            "set"
                + entry.getKey().substring(0, 1).toUpperCase(Locale.US)
        ...
        if (!found) {
          throw new NoSuchMethodException(
              "Expected setter named '" + methodName + "' for var '" + entry.getKey() + "'");
        }
      }
    }
    return instance;
