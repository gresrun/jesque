...
public final class ReflectionUtils {
     ...
     * @throws NoSuchConstructorException if there is not a constructor that matches the given
     *         arguments
     * @throws AmbiguousConstructorException if there is more than one constructor that matches the
     *         given arguments
     * @throws ReflectiveOperationException if any of the reflective operations throw an exception
     */
    ...
    /**
     * Create an object of the given type using a constructor that matches the supplied arguments
     * and invoke the setters with the supplied variables.
     * 
     * @param <T> the object type
     * @param clazz the type to create
     * @param args the arguments to the constructor
     * @param vars the named arguments for setters
     * @return a new object of the given type, initialized with the given arguments
     * @throws NoSuchConstructorException if there is not a constructor that matches the given
     *         arguments
     * @throws AmbiguousConstructorException if there is more than one constructor that matches the
     ...
     */
    public static <T> T createObject(final Class<T> clazz, final Object[] args,
            final Map<String, Object> vars) throws NoSuchConstructorException,
    ...
    private static final class ConstructorComparator
        ...
        public int compare(final Constructor<?> c1, final Constructor<?> c2) {
            ...
            return (p1 != p2) ? (p1 ? -1 : 1)
                    : Integer.valueOf(c2.getParameterTypes().length)
                            .compareTo(c1.getParameterTypes().length);
        }
    }

    /**
     * Return the default ClassLoader to use: typically the thread context ClassLoader, if
     * available; the ClassLoader that loaded the ReflectionUtils class will be used as fallback.
     * <p>
     ...
     * <p>
     * Always uses the default class loader: that is, preferably the thread context class loader, or
     * the ClassLoader that loaded the ReflectionUtils class as fallback.
     * 
     ...
     * <p>
     * Also supports the JVM's internal class names for primitive arrays. Does <i>not</i> support
     * the "[]" suffix notation for primitive arrays; this is only supported by
     ...
     * @return the instance
     * @throws ReflectiveOperationException if there was a problem finding or invoking a setter
     *         method
     */
        ...
        if (instance != null && vars != null) {
            ...
            for (final Entry<String, Object> entry : vars.entrySet()) {
                final String methodName =
                        "set" + entry.getKey().substring(0, 1).toUpperCase(Locale.US)
