...
public class ConcurrentHashSet<E> implements ConcurrentSet<E>, Serializable {
    ...
    /**
     * Creates a new set with the same entries as the given collection. The set is created with a
     * capacity of 1.5 times the number of entries in the given collection or 16 (whichever is
     * greater), and a default load factor (0.75) and concurrencyLevel (16).
     * 
    ...
    public ConcurrentHashSet(final Collection<? extends E> collection) {
        ...
        this.delegate = new ConcurrentHashMap<E, Nothing>(
                Math.max(16, Math.round(collection.size() * 1.5f)));
