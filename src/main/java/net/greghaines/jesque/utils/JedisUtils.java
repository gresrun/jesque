...
public final class JedisUtils {
  ...
  public static boolean reconnect(
      final Jedis jedis, final int reconAttempts, final long reconnectSleepTime) {
    int i = 1;
    do {
      try {
        jedis.disconnect();
  ...
  public static boolean isRecurringQueue(
      final JedisCommands jedis, final String queueKey, final String hashKey) {
  ...
  public static boolean canUseAsRecurringQueue(
      final JedisCommands jedis, final String queueKey, final String hashKey) {
    final String hashType = jedis.type(hashKey);
    return (canUseAsDelayedQueue(jedis, queueKey)
        && (HASH.equalsIgnoreCase(hashType) || NONE.equalsIgnoreCase(hashType)));
  }
