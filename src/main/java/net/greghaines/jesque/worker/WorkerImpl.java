...
public class WorkerImpl implements Worker {
   ...
   *
   * <p><strong>Warning: Enabling this feature is very expensive CPU-wise!</strong><br>
  ...
  public void run() {
    if (this.state.compareAndSet(NEW, RUNNING)) {
      try {
        ...
        this.jedis.set(
            key(WORKER, this.name, STARTED), new SimpleDateFormat(DATE_FORMAT).format(new Date()));
  ...
  public void setQueues(final Collection<String> queues) {
    checkQueues(queues);
    this.queueNames.clear();
    this.queueNames.addAll(
        (queues == ALL_QUEUES) // Using object equality on purpose
  ...
  protected String pop(final String curQueue) {
    ...
    switch (nextQueueStrategy) {
      ...
      case RESET_TO_HIGHEST_PRIORITY:
        return (String)
            this.jedis.evalsha(
                this.multiPriorityQueuesScriptHash.get(), 3, curQueue, inflightKey, namespace, now);
      default:
        throw new RuntimeException("Unimplemented 'nextQueueStrategy'");
    }
  }
  ...
  private void loadRedisScripts() throws IOException {
    ...
    this.multiPriorityQueuesScriptHash.set(
        this.jedis.scriptLoad(ScriptUtils.readScript(POP_FROM_MULTIPLE_PRIO_QUEUES)));
  }
