...
public class WorkerPoolImpl implements Worker {
  ...
  public void run() {
    if (this.state.compareAndSet(NEW, RUNNING)) {
      try {
        ...
        this.jedisPool.set(
            key(WORKER, name, STARTED), new SimpleDateFormat(DATE_FORMAT).format(new Date()));
  ...
  private void removeInFlight(
      final UnifiedJedis jedis, final String curQueue, boolean skipRequeue) {
  ...
  protected void loadRedisScripts(final UnifiedJedis jedis) throws IOException {
    ...
    multiPriorityQueuesScriptHash.set(
        jedis.scriptLoad(ScriptUtils.readScript(POP_FROM_MULTIPLE_PRIO_QUEUES)));
  }
