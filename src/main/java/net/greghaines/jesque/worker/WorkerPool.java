...
/**
 * WorkerPool creates a fixed number of identical <code>Workers</code>, each on a separate <code>
 * Thread</code>.
 */
public class WorkerPool implements Worker {
   ...
   */
  public WorkerPool(
      final Callable<? extends Worker> workerFactory,
  ...
  /**
   * Shutdown this pool and wait millis time per thread or until all threads are finished if millis
   * is 0.
   *
   * @param now if true, an effort will be made to stop any jobs in progress
   * @param millis the time to wait in milliseconds for the threads to join; a timeout of 0 means to
   *     wait forever.
