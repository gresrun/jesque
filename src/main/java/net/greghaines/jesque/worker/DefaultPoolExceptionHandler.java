 ...
 * exception was JSON-related or a thread interrupt and terminates if the executor is shutdown.
 */
public class DefaultPoolExceptionHandler implements ExceptionHandler {
	...
	public RecoveryStrategy onException(final JobExecutor jobExecutor, final Exception exception,
		...
		final boolean isLoadingDataset = exception instanceof JedisDataException && exception
				.getMessage().startsWith("LOADING Redis is loading the dataset in memory");

		if (exception instanceof JedisConnectionException
				|| exception instanceof JedisNoScriptException || isLoadingDataset) {
			return RECONNECT;
