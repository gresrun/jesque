...
/**
 * DefaultExceptionHandler reconnects if there is a connection exception, proceeds if the exception
 * was JSON-related or a thread interrupt and terminates if the executor is shutdown.
 */
public class DefaultExceptionHandler implements ExceptionHandler {
	...
	public RecoveryStrategy onException(final JobExecutor jobExecutor, final Exception exception,
		...
		return (exception instanceof JedisConnectionException) ? RECONNECT
				: (((exception instanceof JsonProcessingException)
						|| ((exception instanceof InterruptedException)
								&& !jobExecutor.isShutdown())) ? PROCEED : TERMINATE);
	}
}
