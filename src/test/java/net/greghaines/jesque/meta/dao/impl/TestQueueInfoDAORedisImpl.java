...
public class TestQueueInfoDAORedisImpl {
    ...
    public void testGetPendingCount() {
        ...
        queueTypeMap.put("queue1", REGULAR_QUEUE);
        queueTypeMap.put("queue2", DELAYED_QUEUE);
        this.mockCtx.checking(new Expectations() {{
            ...
            for (final Entry<String, String> e : queueTypeMap.entrySet()) {
                ...
                oneOf(jedis).hget(QUEUE_TYPES, queueKey);
                will(returnValue(e.getValue()));
                if (DELAYED_QUEUE.equals(e.getValue())) {
    ...
    public void testGetQueueInfos() {
        ...
        this.mockCtx.checking(new Expectations() {{
            ...
            for (final Entry<String, String> e : queueTypeMap.entrySet()) {
                ...
                oneOf(jedis).hget(QUEUE_TYPES, queueKey);
                will(returnValue(e.getValue()));
                if (DELAYED_QUEUE.toString().equals(e.getValue())) {
                    oneOf(jedis).zcard(queueKey);
                    will(returnValue(queueCountMap.get(e.getKey())));
                } else {
                    oneOf(jedis).llen(queueKey);
    ...
    @Test
    public void testGetQueueInfo_Regular() throws JsonProcessingException {
        ...
        this.mockCtx.checking(new Expectations() {{
            ...
            will(returnValue(jedis));
            exactly(2).of(jedis).hget(QUEUE_TYPES, queueKey);
            will(returnValue(REGULAR_QUEUE));
            oneOf(jedis).llen(queueKey);
    ...
    @Test
    public void testGetQueueInfo_Delayed() throws JsonProcessingException {
        final String name = "queue1";
        final String queueKey = "resque:queue:" + name;
        final long jobOffset = 1;
        final long jobCount = 2;
        ...
        this.mockCtx.checking(new Expectations() {{
            ...
            will(returnValue(jedis));
            exactly(2).of(jedis).hget(QUEUE_TYPES, queueKey);
            will(returnValue(DELAYED_QUEUE));
            oneOf(jedis).zcard(queueKey);
            will(returnValue(size));
            oneOf(jedis).zrange(queueKey, jobOffset, jobOffset + jobCount - 1);
            will(returnValue(payloads));
            oneOf(jedis).close();
        }});
        final QueueInfo queueInfo = this.qInfoDAO.getQueueInfo(name, jobOffset, jobCount);
