...
public final class TestUtils {
  ...
  public static void delayEnqueueJobs(
      final String queue, final List<Job> jobs, final Config config) {
    final Client client = new ClientImpl(config);
    try {
      int i = 1;
      for (final Job job : jobs) {
  ...
  public static void removeDelayEnqueueJobs(
      final String queue, final List<Job> jobs, final Config config) {
    final Client client = new ClientImpl(config);
    try {
      for (final Job job : jobs) {
  ...
  public static JobFactory createTestActionJobFactory() {
    return new MapBasedJobFactory(Map.of(TestAction.class.getSimpleName(), TestAction.class));
  }
