...
public class IntegrationTest {
  ...
  public void unpermittedJob() {
    ...
    doWork(
        Arrays.asList(job),
        Map.of(FailAction.class.getSimpleName(), FailAction.class),
        new WorkerListener() {
          public void onEvent(
  ...
  private static void assertMixed(final WorkerListener listener, final WorkerEvent... events) {
    ...
    doWork(
        Arrays.asList(job1, job2, job3, job4),
        Map.of(
            FailAction.class.getSimpleName(),
            FailAction.class,
            TestAction.class.getSimpleName(),
            TestAction.class),
        listener,
  ...
  private static void doBatchWork(
      final List<Job> jobs,
      final Map<String, ? extends Class<? extends Runnable>> jobTypes,
      final WorkerListener listener,
      final WorkerEvent... events) {
