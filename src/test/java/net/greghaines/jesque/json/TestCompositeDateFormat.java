...
//        }
	...
	public void testParse_PHP() throws ParseException {
		...
		// Since PHP's format is missing milliseconds, see if they're within a second of each other
	...
	public void testParse_Ruby() throws ParseException {
		...
		// Since Ruby's formats are missing milliseconds, see if they're within a second of each other
		...
		assertWithinASecond(date, dateFormat.parse("2013-03-07 21:26:05"));
		assertNotWithinASecond(date, dateFormat.parse("2013-03-08 02:26:05")); // No time zone info
		assertWithinASecond(date, dateFormat.parse("2013/03/07 21:26:05 -0500"));
		assertWithinASecond(date, dateFormat.parse("2013/03/08 02:26:05 +0000"));
		assertWithinASecond(date, dateFormat.parse("2013/03/07 21:26:05"));
		assertNotWithinASecond(date, dateFormat.parse("2013/03/08 02:26:05")); // No time zone info
	...
	private static void assertNotWithinASecond(final Date expected, final Date actual) {
		Assert.assertFalse(Math.abs(expected.getTime() - actual.getTime()) < 1000);
	}
}
